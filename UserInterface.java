...
public class UserInterface<V extends View> {
    ...
    public void start() {
        ...
        while (next) {
            String input = view.getInputData(
                ">\nВведите через пробел: Фамилию, Имя, Отчество, Дату Рождения (в формате dd.mm.yyyy), Номер Телефона(в формате 9115550011), Пол(m,f)\n" +
            ...
            if (input.contains("Exit") || input.contains("exit")) {
                ...
                if (inputDataCount == -1) {
                    view.printOutputData("Введено мало данных. Должно быть: Фамилия, Имя, Отчество, Номер Телефона, Дата Рождения, Пол (разделенные пробелом ' ')\n");
                } else if (inputDataCount == 0) {
                    view.printOutputData("Введено много данных. Должно быть: Фамилия, Имя, Отчество, Номер Телефона, Дата Рождения, Пол (разделенные пробелом ' ')\n");
                } else {
                    try {
                        model = new CheckData();
    ...
    private void writePersonData(CheckData data) throws IOException {
        File filepath = new File("C://Users//sevas//Desktop//HW_3//" + data.getLastName()+".txt");
